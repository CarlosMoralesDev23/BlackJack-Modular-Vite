
// Este archivo deberia llamarse index.js   por convencion. Pero para recordarme que es, le estoy llamando barril, que es su funcion. 

export { condicionPuntajeJugador } from "./condicion-puntaje-jugador.js"
export { crearCarta }              from "./crear-carta"
export { crearDeck }               from "./crear-deck";
export { pedirCarta }              from "./pedir-carta";
export { puntosYAsignar }          from "./puntosYAsignar.js"
export { turnoComputadora }        from "./turno-computadora";
export { valorCarta }              from "./valor-carta";

export { btnPedir }                from "./dom.js";
export { btnDetener }              from "./dom.js";
export { btnNuevo }                from "./dom.js";
export { divCartasJugador }        from "./dom.js";
export { divCartasComputadora }    from "./dom.js";
export { puntosHTML }              from "./dom.js";